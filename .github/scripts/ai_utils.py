...
class GLMProvider(ModelProvider):
    ...
    def __init__(self):
        ...
        logger.info("GLM Provider initialized: model=%s, base_url=%s", self.model, base_url)
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                logger.error("All %d attempts failed. Last error: %s", max_retries + 1, e)
                raise
            ...
            logger.warning("Attempt %d failed: %s. Retrying in %.2fs...",
                           attempt + 1, e, sleep_time)
            time.sleep(sleep_time)

    raise last_exception  # Should be unreachable but satisfies linter
...
def parse_json_response(text: str) -> Dict[str, Any]:
    ...
    try:
        import json_repair  # pylint: disable=import-outside-toplevel
